    ...
    <!-- Favicons -->
    <link rel="shortcut icon" type="image/png" href="../assets/img/s9/favicon.png">
    <!-- Font -->
    ...
    <?php
        ...
        $query_get_bank_row = "SELECT * FROM withdrawal_bank WHERE f_member_id='$member_id' AND f_currency_id = '$member_currency_code' AND f_delete = 'N'";
        ...
        $query_get_withdraw_ticket = "SELECT * FROM withdrawal WHERE f_member_id='$member_id' AND f_status = 'A' AND f_delete = 'N'";
        $result_get_withdraw_ticket = mysqli_query($conn, $query_get_withdraw_ticket);
        $num_get_withdraw_ticket = mysqli_num_rows($result_get_withdraw_ticket);

        
        if($num_get_withdraw_ticket > 0){
            $verify_user_bank = "#08fc04";
        }else{
            $verify_user_bank = "#cbcbcb";
        }
        ...
        if (isset($_SESSION['member_id'])) {
           ...
           $check_user_token = "SELECT * FROM member_token where f_username = '$member_username' AND f_delete = 'N'";
            ...
            // add hide class
            $hide= "hide";
            echo '<input type="text" id="session_lang" value="'.$_SESSION["language"].'" hidden  readonly>';
        } else {
            $show1= "hidden";
            // header("Location: ../");
            $annoucement_pop_up = "";
